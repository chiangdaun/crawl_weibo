...
class sinaSpider(Spider):
    ...
    def transfer_pubtime(self,dates):
        if '今天' in dates:
            date = time.strftime("%Y-%m-%d", time.localtime())
            temp = re.search(r'今天(.*)', dates).group(1)
            dates = date + temp + ':00'
        elif '分钟前' in dates:
            tmin = 60 * int(time.strftime("%H", time.localtime())) + int(time.strftime("%M", time.localtime()))
            tmin = tmin - int(re.search(r'.*?(\d+)分钟前', dates).group(1))
            dates = time.strftime("%Y-%m-%d", time.localtime()) + ' ' + str(tmin // 60) + ":" + str(tmin % 60) + ':00'
        elif '月' in dates and '年' not in dates:
            year = time.strftime("%Y", time.localtime())
            date = year + '年' + dates
            dates = date.replace(r'年', '-').replace(r'月', '-').replace(r'日', ' ') + ':00'
        else:
            dates = dates
        return dates
